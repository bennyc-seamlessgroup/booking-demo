document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderSuccessScreen(pickupNumber) {
    const t = I18N[currentLang];
    const total = getTotal();
    const chosenItems = menu.filter(item => (cart[item.id] || 0) > 0);
    ...
    els.pickupNumberValue.textContent = pickupNumber;
  ...
  els.payBtn.addEventListener("click", () => {
    ...
    fetch("https://bennyseamlessgroup.app.n8n.cloud/webhook/takeaway-submit", {
      ...
      .then(async (res) => {
        try {
          ...
          console.log("Webhook returned non-JSON");
        }
      })
      .catch((e) => {
    ...
    setTimeout(() => {
      renderSuccessScreen(pickupNumber);
      showScreen("success");
    }, 1500);
  });
  ...
  els.langToggle.addEventListener("click", () => {
    ...
    renderHeader();
    renderMenu();
    if (!els.cartScreen.classList.contains("hidden")) {
      renderCartScreen();
    }
    if (!els.successScreen.classList.contains("hidden")) {
      renderSuccessScreen(els.pickupNumberValue.textContent || generatePickupNumber());
    }
  });
